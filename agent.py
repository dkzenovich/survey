импортировать JSON
импортировать регистрацию
от langchain_anthropic импортировать ЧатАнтропный
от langchain_core.подсказки импортировать ЧатПодсказкаШаблон
от печати импортировать Список, Диктовка, Любой

АНТРОПИК_API_KEY = "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"
llm_s = ЧатАнтропный(api_key=ANTHROPIC_API_KEY, имя_модели='Клод-3-5-сонет-20240620') #  Клод-3-хайку-20240307

регистрация.базовая внешний Вид(уровень=ведение журнала.отлаживать)

защита агент(пользовательский_вход) -> Дикт[ул, Любая]:
    Agent_prompt = ChatPromptTemplate.from_messages([(
        "система",
        """Вы - эксперт по анализу бизнес-проблем. Ваша задача - внимательно изучить запрос пользователя и выявить основные проблемы, которые необходимо решить..
Следите за этими молодыми людьми:

Внимательно прочитайте предоставленный запрос.
Определите основные проблемы, упомянутые в тексте. Обратите особое внимание:
...
Любые другие важные проблемы в бизнесе


Составьте список из 1-3 основных проблем, которые вы выявили.
Сформулируйте каждую задачу кратко и четко, используя 1 предложение..
